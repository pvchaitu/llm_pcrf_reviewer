...
from pcrf_core import set_reproducibility, compute_hallucination_risk
...
def main(run_mode: str = "full", dataset_path: str = None):
    ...
    # Protected Router execution using contract-clean logic (Phase 1 Fix 4)
    ...
    # Gather precise SFT exposure control stats (Phase 1 Fix 4)
    ...
    # PART J — Router metric audit logs (J3)
    logger.info(f"[ROUTER AUDIT] baseline served count: {sum(1 for r in trace_rows if r['router_decision'] == 'use_baseline')}")
    logger.info(f"[ROUTER AUDIT] candidate served count: {sum(1 for r in trace_rows if r['router_decision'] == 'use_candidate')}")
    logger.info(f"[ROUTER AUDIT] abstain count: {sum(1 for r in trace_rows if r['router_decision'] == 'abstain_safe_fallback')}")
    logger.info(f"[ROUTER AUDIT] semantic recoveries observed: {hallucination_stats_tmp.get('semantic_repairs_observed', 0)}")
    logger.info(f"[ROUTER AUDIT] contract clean repairs promoted: {hallucination_stats_tmp.get('contract_clean_repairs_promoted', 0)}")
    logger.info(f"[ROUTER AUDIT] semantic recoveries withheld: {hallucination_stats_tmp.get('semantic_repairs_withheld_for_contract', 0)}")
...
* Verification Status     : SFT candidate model evaluation processed for {reg_decision.status} route.
...
""")
        for r in trace_rows:
            ...
            f.write(f"SFT Candidate Output: {get_public_candidate_output(safe_r, pcrf_config)}\n")
            ...
            if pcrf_config.artifact_cfg.include_raw_generation_outputs_in_debug_artifacts:
                f.write(f"Raw Baseline Output [Internal SFT Debug Only]: {safe_r.get('raw_baseline_output')}\n")
                f.write(f"Raw Candidate Output [Internal SFT Debug Only]: {safe_r.get('raw_candidate_output')}\n")
            ...
            f.write(f"SFT Candidate Risk Score: {r['candidate_hallucination_risk_score']:.5f}\n")
...
""")
    ...
    with open(final_decision_path, 'w', encoding='utf-8') as f:
        json.dump({
            ...
            "recommended_next_action": "Enable the Canary Protected Router fallback pipeline; SFT weights promotion blocked." if blocking_reasons else "Promote SFT regularizations directly into production."
        }, f, indent=4)
    ...
    if overall_adoption_score >= 80.0 and not blocking_reasons:
        ...
        color_code = "🔴 [HIGH OVER-STEERING RISK | BLOCK SFT MUTATIONS]"
