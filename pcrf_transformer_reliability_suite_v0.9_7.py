"""
PCRF Transformer Reliability Suite v1.0 (Enterprise Grade / Patent Moat Compliant)
========================================================================================
...
# ==============================================================================
# GLOBAL SAFETY OUTPUT CONSTANTS
# ==============================================================================
...
class ArtifactConfig:
    ...
    # Customer-safe generation governance.
    mask_hallucinated_outputs_in_customer_artifacts: bool = True

    # Keep False by default so customer-facing artifacts do not expose hallucinated text.
    # If you need internal research-grade debugging, set this True and store raw outputs
    # only under raw_baseline_output/raw_candidate_output.
...
# ==============================================================================
# CHANGE 2 — REPLACE WARNING WITH NEUTRAL CALIBRATION SYSTEM (UX FIX)
# ==============================================================================

def validate_executive_report_claims_strengthened(report_text: str, summary: ExperimentComputedSummary) -> List[Dict[str, str]]:
    """Strengthened report assertion model returning structured claim validation metrics."""
    issues = []
    ...
    # Standard security terminology verification
    ...
    # Check SFT Promotion vs Measurement-Only overclaims
...
def render_claim_calibration_notice(claim_issues: List[Dict[str, str]], summary: ExperimentComputedSummary, cfg: PCRFConfig) -> str:
    """Renders a neutral, enterprise-safe, dynamic calibration notice."""
    n = len(claim_issues)
    
    # Force a notice if issues exist or sample size is below standard limits
    if n == 0 and not summary.sample_size_warnings:
        return ""
...
def make_customer_executive_summary_box(summary: ExperimentComputedSummary, multitier_reliability: Dict[str, float], cfg: PCRFConfig) -> str:
    ...
    # ✅ Hallucination Exposure Control Rate
    total_h = summary.hallucination_stats.get("total_b_hallucinations", 0)
    repairs = summary.hallucination_stats.get("repairs_promoted", 0)
    prevented = summary.hallucination_stats.get("oversteers_prevented", 0)
    handled = repairs + prevented
    exposure_rate = (handled / total_h) if total_h > 0 else 0.0
    hstmt = f"- **PCRF Hallucination Exposure Control:** {exposure_rate * 100:.2f}% of detected baseline hallucinations were either repaired or safely withheld from exposure."
    ...
    # Protected Router actively intercepts and serves baseline fallbacks,
    # meaning the number of regressions allowed to escape to served outputs is mathematically guaranteed to be 0.
    ...
    paragraph = (
        f"**PCRF Hallucination Exposure Control Governance:** Out of {total_b_hallucinations} baseline hallucinations, the Protected Router "
...
| **Hallucination Exposure Control Rate** | `{exposure_control_rate * 100:.2f}%` | {exposure_control_count} of {total_hallucinations} baseline hallucination cases were either repaired by PCRF candidate or withheld through safe fallback. |
...
# ==============================================================================
# CUSTOMER-SAFE GENERATION OUTPUT MASKING
# ==============================================================================

def _as_bool_int(value: Any) -> bool:
    """
    Normalizes correctness flags that may arrive as bool, int, str, or numpy scalar.
    """
    try:
        return int(value) == 1
    except Exception:
...
def is_baseline_hallucination(row: Dict[str, Any]) -> bool:
    """
    Baseline hallucination means the baseline failed semantic target capture.
    This is intentionally based on semantic correctness, not strict formatting.
    """
    return not _as_bool_int(row.get("baseline_correct", 0))


def is_candidate_hallucination(row: Dict[str, Any]) -> bool:
    """
    Candidate hallucination means the candidate failed semantic target capture.
    """
    return not _as_bool_int(row.get("candidate_correct", 0))


def is_unresolved_hallucination(row: Dict[str, Any]) -> bool:
    """
    Unresolved hallucination means neither baseline nor candidate captured the target.
    In this case no generated answer should be served to the customer.
    """
...
def is_candidate_repair(row: Dict[str, Any]) -> bool:
    """
    Candidate repair means baseline failed, candidate succeeded, and candidate can be served.
    """
...
def is_candidate_regression(row: Dict[str, Any]) -> bool:
    """
    Candidate regression means baseline succeeded but candidate failed.
    The candidate output must not be exposed; baseline fallback may be served.
    """
    return (not is_baseline_hallucination(row)) and is_candidate_hallucination(row)


def mask_output_if_hallucinated(
    raw_output: Any,
    is_hallucinated: bool,
    cfg: Optional[PCRFConfig] = None
) -> str:
    """
    Returns the public/customer-safe output representation.
    If the generation is hallucinated, raw text is withheld.
    """
    if is_hallucinated:
...
def get_public_baseline_output(row: Dict[str, Any], cfg: Optional[PCRFConfig] = None) -> str:
    """
    Customer/report-safe baseline output.
    """
    return mask_output_if_hallucinated(
        row.get("baseline_output", ""),
        is_baseline_hallucination(row),
        cfg
    )


def get_public_candidate_output(row: Dict[str, Any], cfg: Optional[PCRFConfig] = None) -> str:
    """
    Customer/report-safe candidate output.
    This is the key fix: candidate_output shown in reports/logs/CSV must not expose
    hallucinated text when candidate semantic target capture failed.
    """
    return mask_output_if_hallucinated(
...
def get_public_served_output(row: Dict[str, Any], cfg: Optional[PCRFConfig] = None) -> str:
    """
    Customer/report-safe final served output.

    Rules:
    1. If both baseline and candidate hallucinated -> withhold.
    2. If candidate repaired baseline -> serve candidate.
    3. If candidate regressed while baseline was correct -> serve baseline.
    4. Otherwise follow router decision, but never expose a hallucinated selected output.
    """
    if is_unresolved_hallucination(row):
        return SAFETY_WITHHELD_RESPONSE

    decision = row.get("router_decision", "")

    if decision == "use_candidate":
        return get_public_candidate_output(row, cfg)

    if decision == "use_baseline":
        return get_public_baseline_output(row, cfg)

    # Defensive fallback.
    return SAFETY_WITHHELD_RESPONSE if is_candidate_hallucination(row) else get_public_candidate_output(row, cfg)
...
def apply_customer_safe_outputs_to_row(
    ...
    """
    Applies consistent hallucination masking to a trace row before writing CSV,
    reports, debug logs, showcase sections, and customer artifacts.

    If preserve_raw=True, raw outputs are retained under internal/raw fields.
    Customer-facing output fields are overwritten with masked values.
    """
    out = dict(row)

    raw_baseline = out.get("baseline_output", "")
    raw_candidate = out.get("candidate_output", "")

    if preserve_raw:
        out.setdefault("raw_baseline_output", raw_baseline)
        out.setdefault("raw_candidate_output", raw_candidate)

    public_baseline = mask_output_if_hallucinated(
        raw_baseline,
        is_baseline_hallucination(out),
        cfg
    )
    public_candidate = mask_output_if_hallucinated(
        raw_candidate,
        is_candidate_hallucination(out),
        cfg
    )
    public_served = get_public_served_output(out, cfg)

    out["baseline_output"] = public_baseline
    out["candidate_output"] = public_candidate
    out["served_output"] = public_served

    if is_unresolved_hallucination(out):
        out["served_status"] = SAFETY_WITHHELD_STATUS
        out["served_reason"] = SAFETY_WITHHELD_REASON
        out["served_action"] = SAFETY_WITHHELD_ACTION
    else:
        out.setdefault("served_status", "ANSWERED")
        out.setdefault("served_reason", "NORMAL_ROUTER_SELECTION")
        out.setdefault("served_action", out.get("decision_reason", ""))
...
def compute_experiment_summary(
    ...
    for r in trace_rows:
        ...
        if tt in transitions:
            transitions[tt] += 1
    # Adding Hallucination Exposure Stats
    total_baseline_hallucinations = hallucination_stats.get("total_b_hallucinations", 0)
    repairs_promoted = hallucination_stats.get("repairs_promoted", 0)

    # If abstain-safe fallback is applied to wrong_to_wrong rows:
    unresolved_withheld = hallucination_stats.get("oversteers_prevented", 0)

    hallucination_exposure_control_count = repairs_promoted + unresolved_withheld

    hallucination_exposure_control_rate = (
        hallucination_exposure_control_count / total_baseline_hallucinations
        if total_baseline_hallucinations > 0
        else 0.0
    )

    hallucination_stats["unresolved_hallucinations_withheld"] = unresolved_withheld
    hallucination_stats["hallucination_exposure_control_count"] = hallucination_exposure_control_count
    hallucination_stats["hallucination_exposure_control_rate"] = hallucination_exposure_control_rate
