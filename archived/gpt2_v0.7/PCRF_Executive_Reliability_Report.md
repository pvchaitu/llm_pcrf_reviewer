...
| **Seen Validation Loss (NLL)** | 4.76438 | 4.64522 | -0.11916 |
| **Unseen Validation Loss (NLL)** | 5.19742 | 5.12605 | -0.07137 |
| **Unseen Perplexity (PPL)** | 180.81 | 168.35 | -12.45 |
...
| Structural Depth Monitor | Unmonitored Depth | Chain Reliability: 99.55% | 99.5/100 | `MEASUREMENT_ONLY - ROADMAP GATED` |
...
|---|---|---|---|---|---|---|
| seen_val | 84 | *Complete with one word only: The official capital city of Switzerland is* | `Bern` | `Zurich.

The Swiss` | `Zurich.

The Swiss` | 4.3837 |
| seen_val | 86 | *Complete with one word only: The noble element designated by atomic number 10 is* | `Neon` | `the most important element in the` | `the most important element in the` | 13.5739 |
| seen_val | 87 | *Complete with one word only: The volatile element designated by atomic number 16 is* | `Sulfur` | `the most volatile element in the` | `the most volatile element in the` | 5.3019 |
| seen_val | 88 | *Complete with one word only: The chemical molecule animals must extract from air to survive is* | `Oxygen` | `called the "air molecule."` | `called a "chemical-free` | 3.9075 |
| seen_val | 89 | *Complete with one word only: The yellow dwarf star supporting life at the center of our solar system is the* | `Sun` | `most massive star in the universe` | `closest thing to a star we` | 3.9867 |
| seen_val | 90 | *Complete with one word only: Mechanical acoustics are completely incapable of moving across a spatial* | `Vacuum` | `space.

The first` | `space.

The first` | 7.2123 |
| seen_val | 96 | *Complete with one word only: To enforce unique constraints with no duplicated items, algorithms utilize a* | `Set` | `single word.

The` | `single word.

The` | 7.2777 |
| seen_val | 97 | *Complete with one word only: The hypermedia syntax used to format layout documents across the World Wide Web is* | `HTML` | `now supported.

The` | `now supported.

The` | 7.7287 |
| seen_val | 98 | *Complete with one word only: An execution failure originating from incorrect program logic is called a* | `Bug` | `"failure of execution".` | `"failure".` | 10.1821 |
| seen_val | 99 | *Complete with one word only: A standardized text notation representing complex structural records is* | `JSON` | `used.
