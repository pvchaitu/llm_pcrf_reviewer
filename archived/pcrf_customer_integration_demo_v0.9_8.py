...
def main():
    ...
    for idx, ex in enumerate(all_val_dataset_prompts):
        ...
        # ✅ Apply output masking consistently (POINT 4)
    ...
    # ✅ Handle validation trace mapping (POINT 10)
    ...
    with open(failed_trace_csv, 'w', newline='', encoding='utf-8') as f:
        if customer_safe_trace_rows:
            # Exclude raw baseline outputs to ensure security boundary
    ...
    # Form intermediate stats
    hallucination_stats_tmp = {
        "avg_b_hr": avg_b_hr,
        "avg_c_hr": avg_c_hr,
    ...
    # Run dynamic exposure control audits before saving scorecard reports (POINT 11)
    exp_stats = compute_hallucination_exposure_control_stats(trace_rows)
    hallucination_stats_tmp.update(exp_stats)
    ...
    # Detailed debug text logger (pcrf_debug_report.txt - POINT 9)
...
""")
        for r in trace_rows:
            ...
            # Mask debug outputs consistently based on safe artifacts policy
            ...
            if is_unresolved_hallucination(safe_r):
