...
class RegularizationConfig:
    ...
    lambda_contrastive: float = 0.5  # Contrastive Formatting Suppressor Strength
...
class StructuralPCRFPlugin(BaseFeaturePlugin):
    ...
    def run_standalone(self, model: PreTrainedModel, tokenizer: PreTrainedTokenizer, splits: Dict[str, List[ClozeQAExample]], cfg: PCRFConfig) -> Any:
        ...
        for idx, r_l in enumerate(layer_reliabilities):
            ...
            # Measures downstream system reliability gradient scaled by active sublayer energy
    ...
    def should_apply(self, baseline_stats: Dict[str, Any], layer_breakdown: List[Dict[str, Any]], gate_cfg: PromotionGateConfig) -> FeatureDecisionReport:
        # Multi-Tier Scorecard Gating (Requirement 6)
        r_sys_series = np.prod([x["r_series_local"] for x in layer_breakdown])
        r_sys_crew_geo = float(math.exp(np.mean([math.log(max(1e-12, x["reliability_r_l"])) for x in layer_breakdown])))
        
        # Geometrically normalized CREW health score must exceed threshold (e.g. 95%)
...
class DerivativeRegularizer(BaseFeaturePlugin):
    ...
    def run_standalone(self, model: PreTrainedModel, tokenizer: PreTrainedTokenizer, splits: Dict[str, List[ClozeQAExample]], cfg: PCRFConfig) -> Any:
        ...
        for batch in loader:
            ...
            # 6. Contrastive Decoding Regularization (CDL v2 formatting suppressor)
            # Evaluated strictly in probability space for scaling stability (Issue B)
            ...
            # 3. L_margin: Logit Margin Amplification
...
class SafePCRFController:
    ...
    def compute_promotion_decision(self, baseline_metrics: Dict[str, Any], feature_metrics: Dict[str, Any], feature_name: str, r_sys_chain: float = 1.0) -> FeatureDecisionReport:
        # Gating Loophole (Structural Decay Overrule) (Fix 1)
        ...
        if path_c_active:
            ...
            if not is_stat_significant:
                return FeatureDecisionReport(
                    ...
                    explanation="Confidence intervals for validation NLL drop crosses zero boundary or exceeds significance bounds.",
