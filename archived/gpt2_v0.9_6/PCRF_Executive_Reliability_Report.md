...
**PCRF Router (Hallucinations) Governance:** Out of 23 baseline hallucinations, the Protected Router executed 23 net interventions—safely promoting 1 verified candidate repair while blocking 22 high-confidence candidate over-steers. By enforcing a perfect safety ceiling with 0 regressions allowed to escape to served outputs, PCRF secures model upgrades with zero downtime or regression risk.
...
| Seen Validation NLL | Lower is Better ⬇️ | 4.6311 | 4.3625 | 4.6039 | -0.2686 | -0.0272 | Candidate Improved (-0.2686), Served Improved (-0.0272). |
...
| Average Cross-Entropy Loss (NLL) | Lower is Better ⬇️ | 4.8153 | 4.6217 | 4.7963 | -0.1936 | -0.0189 | Candidate Improved (-0.1936), Served Improved (-0.0189). |
...
| **Candidate Over-Steers Prevented** | `22` | Both models failed, but candidate risk was higher; baseline served. |
| **Catastrophic Regressions Blocked** | `0` | Baseline was correct but candidate failed; router served baseline fallback. |
| **Net Gateway Interventions** | `23` | Overall cases actively guarded by the Protected Router (100% active coverage). |
...
| **Over-steers Prevented** | `22` | Fallback to baseline when candidate risk spikes |
...
* **Router Action:** `use_baseline` -> **Served Output:** `Zurich.<br><br>The Swiss capital is the capital of Switzerland`
