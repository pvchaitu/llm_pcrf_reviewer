...
class EvaluatorPlus:
    """Enriched CLM evaluator breaking performance metrics into five distinct corporate dimensions."""
    @staticmethod
    def evaluate_dataset_detailed(
        ...
        with torch.no_grad():
            for batch in loader:
                ...
                # Metric 1: Strict EM (True only when normalized output equals normalized target exactly)
                is_strict_em = (norm_actual == norm_target)
                if is_strict_em:
                    strict_em_count += 1
                    
                # Metric 2: First-Token Match (True when first generated token matches first target token)
                ...
                # Metric 3: Semantic Target Capture (True when target appears anywhere in generated text)
                is_semantic = evaluate_semantic_match(actual_text, original_example.target)
                if is_semantic:
                    semantic_capture_count += 1
                    
                # Metric 4 & 5: Instruction Contract Violation / Over-generation (target is captured but model leaks extra tokens)
                is_instruction_violation = False
                if is_semantic and len(actual_text.split()) > len(original_example.target.split()):
                    is_instruction_violation = True
        ...
        return {
            ...
            "exact_match_acc": acc_semantic, # Legacy EM retained for safety gating
...
class DerivativePlugin(BaseFeaturePlugin):
    ...
    def run_standalone(self, model: PreTrainedModel, tokenizer: PreTrainedTokenizer, splits: Dict[str, List[ClozeQAExample]], cfg: PCRFConfig) -> Any:
        ...
        for l_idx in range(num_layers):
            ...
            # Precise math column terms (Section 2)
            layer_derivatives.append({
                "layer_id": l_idx,
                "clean_mean_target_prob": base_mean,
                "perturbed_mean_target_prob": pert_mean,
                "delta_prob": delta,
                "dr_de": -delta # Sensitivity is negative of derivative w.r.t reliability
            })
...
class CurriculumPlugin(BaseFeaturePlugin):
    ...
    def run_standalone(self, model: PreTrainedModel, tokenizer: PreTrainedTokenizer, splits: Dict[str, List[ClozeQAExample]], cfg: PCRFConfig) -> Any:
        ...
        # Normalize weights for pipeline scalability (Section 9 & 2)
        ...
        # Segment into curriculum sampling buckets based on dynamic prompt count (Requirement 4)
        ...
        # Save complete productized artifacts
        os.makedirs(cfg.artifact_cfg.output_dir, exist_ok=True)
...
# ------------------------------------------------------------------------------
# TRACK (A): STRUCTURAL MONITORS WITH DEPTH-ADAPTIVE BETAS & ENTROPY
# ------------------------------------------------------------------------------
class StructuralPCRFPlugin(BaseFeaturePlugin):
    ...
    def run_standalone(self, model: PreTrainedModel, tokenizer: PreTrainedTokenizer, splits: Dict[str, List[ClozeQAExample]], cfg: PCRFConfig) -> Any:
        ...
        # Depth-Adaptive Calibration Correction to stabilize survival calculations across varying architecture depths [5]
        ...
        # Grab derivatives for detailed metric calculations dynamically using centralized configs
        deriv_plugin = DerivativePlugin()
        ...
        for idx, r_l in enumerate(layer_reliabilities):
            # Formulate Alignment-Aware energy ratio (vector cosine interaction)
            # Estimate energy partition dynamically from attention/mlp block parameters
            block = mgr.block_list[idx] if mgr.block_list is not None else None
            w_attn = 0.30
            w_mlp = 0.50
            if block is not None:
                # Estimate weight parameters strictly based on actual parameter footprints
            ...
            s_l = -math.log(max(1e-12, r_crew)) # Structural entropy
            d_r_crew = derivatives[idx] * (r_crew / max(1e-12, r_l)) # Modulated sensitivity
            delta_prob = deriv_probs.get(idx, 0.0)
            
            # Compute CREW-Birnbaum Drift Sensitivity Index (B_l^CREW)
            ...
            if r_crew < 0.95 or abs(d_r_crew) > 0.05:
                is_target = 1
                intervention_reason = f"High structural entropy ({s_l:.4f}) and high bottleneck sensitivity ({d_r_crew:.4f})"
        ...
        worst_k_risk = float(np.mean([1.0 - x["reliability_r_l"] for x in sorted_by_risk[:4]]))
        ...
        with open(plan_path, 'w', newline='', encoding='utf-8') as f:
            writer = csv.DictWriter(f, fieldnames=[
                ...
                "crew_birnbaum_sensitivity", "intervention_flag", 
    ...
    def should_apply(self, baseline_stats: Dict[str, Any], layer_breakdown: List[Dict[str, Any]], gate_cfg: PromotionGateConfig) -> FeatureDecisionReport:
        # Multi-Tier Scorecard Gating (Part B)
...
class DerivativeRegularizer(BaseFeaturePlugin):
    ...
    def run_standalone(self, model: PreTrainedModel, tokenizer: PreTrainedTokenizer, splits: Dict[str, List[ClozeQAExample]], cfg: PCRFConfig) -> Any:
        ...
        for batch in loader:
            ...
            # Contrastive Decoding Regularization Penalty (CDL v2 formatting suppressor in probability space)
...
# Formatting and Sanitization Helpers for Markdown Tables (Part J)
...
class ExecutiveReportGenerator:
    ...
    def generate_report(
        ...
        # Safe extraction helper variables to prevent NameError/AttributeError (Fix 2)
        reg_seen_acc = regularized_stats.get("seen_val_acc", 0.0) if regularized_stats else 0.0
        reg_unseen_acc = regularized_stats.get("unseen_val_acc", 0.0) if regularized_stats else 0.0
        reg_seen_nll = regularized_stats.get("seen_val_nll", 0.0) if regularized_stats else 0.0
        reg_unseen_nll = regularized_stats.get("unseen_val_nll", 0.0) if regularized_stats else 0.0
        reg_unseen_ppl = regularized_stats.get("unseen_val_ppl", 0.0) if regularized_stats else 0.0
        
        router_seen_acc = reconciliation_data.get("router_seen_accuracy", 0.0)
        router_unseen_acc = reconciliation_data.get("router_unseen_accuracy", 0.0)
        ...
        if os.path.exists(curriculum_path):
            ...
            for row in curriculum_rows[:5]:
                curriculum_summary += f"| {row['id']} | *{sanitize_table_cell(row['prompt'])}* | `{sanitize_table_cell(row['target'])}` | **{float(row['priority_score']):.2f}** |\n"
        ...
        if failed_generations:
            ...
            for row in failed_generations[:10]:
                truncated_prompt = truncate_for_report(row["prompt"], 50)
                failed_generations_table += (
                    f"| {row['split']} | {row['id']} | *{truncated_prompt}* | "
                    f"`{sanitize_table_cell(row['target'])}` | `{truncate_for_report(row['baseline_output'], 30)}` | `{truncate_for_report(row['candidate_output'], 30)}` | {row['baseline_nll']:.4f} |\n"
                )
        ...
        # 6. Extract data for Calibrated Ignorance Trace Showcase (Part C)
        ...
        # Failure Taxonomy Table construction (Part E)
        failure_rows = ""
        for cat, cnt in failure_taxonomy.items():
            example_ids = [str(r["id"]) for r in failed_generations if r["failure_category"] == cat]
            examples_str = ", ".join(example_ids[:5]) if example_ids else "None"
            if len(example_ids) > 5: examples_str += "..."
            
            interpretation = "Model output was absent or corrupted."
            fix = "Increase formatting suppression weights."
            if cat == "HIGH_CONFIDENCE_WRONG":
                interpretation = "Incorrect output generated with low decoding uncertainty."
                fix = "Enable high-confidence penalty regularization loops."
            elif cat == "INSTRUCTION_CONTRACT_VIOLATION":
                interpretation = "Target text detected but accompanied by extraneous output formatting."
                fix = "Scale up Contrastive Decoding Suppression λ_contrastive."
                
            failure_rows += f"| {cat} | {cnt} | {examples_str} | {interpretation} | {fix} |\n"
...
* **Why was Adoption Rejected?**
  Despite surface-level accuracy gains, **adoption was rejected (DO_NOT_APPLY / STRUCTURAL_DECAY_REJECTED)**. Hidden structural representation checks detected that overall system chain reliability ($R_{{sys}}$) collapsed to `{multitier_reliability['series']:.4f}`, falling below the non-negotiable **75.0% structural safety floor**.
* **What did PCRF Prove?**
...
|---|---|---|---|
| **Seen Validation Accuracy** | {baseline_stats.get('seen_val_acc', 0.0)*100:.2f}% | {reg_seen_acc*100:.2f}% | {((reg_seen_acc - baseline_stats.get('seen_val_acc', 0.0))*100):+.2f}% |
| **Unseen Generalization Acc** | {baseline_stats.get('unseen_val_acc', 0.0)*100:.2f}% | {reg_unseen_acc*100:.2f}% | {((reg_unseen_acc - baseline_stats.get('unseen_val_acc', 0.0))*100):+.2f}% |
| **Seen Validation Loss (NLL)** | {baseline_stats.get('seen_val_nll', 0.0):.5f} | {reg_seen_nll:.5f} | {(reg_seen_nll - baseline_stats.get('seen_val_nll', 0.0)):+.5f} |
| **Unseen Validation Loss (NLL)** | {baseline_stats.get('unseen_val_nll', 0.0):.5f} | {reg_unseen_nll:.5f} | {(reg_unseen_nll - baseline_stats.get('unseen_val_nll', 0.0)):+.5f} |
| **Unseen Perplexity (PPL)** | {baseline_stats.get('unseen_val_ppl', 0.0):.2f} | {reg_unseen_ppl:.2f} | {(reg_unseen_ppl - baseline_stats.get('unseen_val_ppl', 0.0)):+.2f} |
...
|---|---:|---|---|
| **Strict Series $R_{{sys}}$** | `{multitier_reliability['series']:.4f}` | Legacy Gate / Diagnostic | Conservative adversarial envelope (worst-case scenario) |
| **CREW Product $R_{{sys}}$** | `{multitier_reliability['crew_prod']:.4f}` | Gate | Residual-aware product reliability |
| **CREW Geometric Reliability** | `{multitier_reliability['crew_geo']:.4f}` | Primary Gate | Depth-normalized structural health (cross-model invariant) |
| **Worst-k CREW Bottleneck Risk** | `{multitier_reliability['worst_k']:.4f}` | Bottleneck Gate | Identifies true target blocks for adaptive regularizers |
...
* **Baseline Output:** `{sanitize_table_cell(p119['b_out'])}` | **Candidate Output:** `{sanitize_table_cell(p119['c_out'])}`
...
* **Gateway Action:** `HallucinationFound: "{sanitize_table_cell(p119['c_out'])}"` -> {sanitize_table_cell(p119['reason'])}

#### CASE 2: Prompt ID 120
* **Prompt:** *{p120['prompt']}*
* **Expected Target:** `{p120['expected']}`
* **Baseline Output:** `{sanitize_table_cell(p120['b_out'])}` | **Candidate Output:** `{sanitize_table_cell(p120['c_out'])}`
...
* **Gateway Action:** `HallucinationFound: "{sanitize_table_cell(p120['c_out'])}"` -> {sanitize_table_cell(p120['reason'])}

#### CASE 3: Prompt ID 110
* **Prompt:** *{p110['prompt']}*
* **Expected Target:** `{p110['expected']}`
* **Baseline Output:** `{sanitize_table_cell(p110['b_out'])}` | **Candidate Output:** `{sanitize_table_cell(p110['c_out'])}`
...
* **Gateway Action:** `HallucinationFound: "{sanitize_table_cell(p110['c_out'])}"` -> {sanitize_table_cell(p110['reason'])}
...
|---|---:|---:|---:|---|
| **Seen Accuracy** | {baseline_stats.get('seen_val_acc', 0.0)*100:.2f}% | {reg_seen_acc*100:.2f}% | {router_seen_acc*100:.2f}% | **100% Regression-Guarded**: Serves baseline outputs where candidate fell back |
| **Unseen Accuracy** | {baseline_stats.get('unseen_val_acc', 0.0)*100:.2f}% | {reg_unseen_acc*100:.2f}% | {router_unseen_acc*100:.2f}% | Serves candidate improvements while blocking regressions |
| **Regressions Blocked** | 0 | 0 | **{reconciliation_data['regressions_blocked']}** | Intercepted correct-to-wrong transitions in real-time |
| **Over-Steers Prevented** | 0 | 0 | **{reconciliation_data['oversteers_prevented']}** | Retained baseline outputs where candidate risk spiked |
| **Repairs Promoted** | 0 | 0 | **{reconciliation_data['repairs_promoted']}** | Approved optimized parameters served to production |
...
*   **Train Examples Partition Count:** `{len(splits["train"])}`
*   **Seen Validation Split Count:** `{len(splits["seen_val"])}`
*   **Unseen Validation Split Count:** `{len(splits["unseen_val"])}`
*   **Combined Validation Evaluated Count:** `{len(splits["seen_val"]) + len(splits["unseen_val"])}`
...
## 13. Actionable 4-Phase Enterprise Upgrade Strategy for {model_name_dynamic} (Part L)
...
"""
    ...
    def _generate_conditional_blueprints(output_dir: str, scorecard: Dict[str, Any], baseline_stats: Dict[str, Any]):
        ...
        # Enable blueprint outputs dynamically for active diagnostic or gating tracks (Part C)
        valid_statuses = ["SAFE_TO_APPLY", "MEASUREMENT_ONLY", "PROMOTED", "PROMOTED_PATH_C"]
        
        # Track 1: Derivatives Blueprint
        if scorecard.get("Derivatives", {}).get("status") in valid_statuses:
...
"""
        ...
        if scorecard.get("Curriculum Curation", {}).get("status") in valid_statuses:
...
"""
        ...
        if scorecard.get("Structural Depth Monitor", {}).get("status") in valid_statuses:
...
"""
        ...
        if scorecard.get("Safe SFT Regularization", {}).get("status") in valid_statuses:
