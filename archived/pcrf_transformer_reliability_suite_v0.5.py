...
"""
PCRF Transformer Reliability Suite v1 (With Advanced Information Gating & Routing)
==================================================================================
...
def generate_mock_cloze_dataset() -> Dict[str, List[ClozeQAExample]]:
    ...
    # Task Type: cs/factual is_critical=1 for highly strategic core domains.
    factual_countries = [
    ...
    for idx, (p, t, task, s, is_c, wt) in enumerate(raw_source):
        example = ClozeQAExample(
            example_id=idx + 1, prompt=p, target=t, task_type=task, split=s,
...
class DerivativePlugin(BaseFeaturePlugin):
    ...
    def run_standalone(self, model: PreTrainedModel, tokenizer: PreTrainedTokenizer, splits: Dict[str, List[ClozeQAExample]], cfg: Any) -> Any:
        ...
        for l_idx in range(num_layers):
            ...
            # Using precise terminology: clean_mean_target_prob, perturbed_mean_target_prob, delta_prob
        ...
        with open(csv_path, 'w', newline='', encoding='utf-8') as f:
            writer = csv.DictWriter(f, fieldnames=["layer_id", "clean_mean_target_prob", "perturbed_mean_target_prob", "delta_prob"])
            writer.writeheader()
...
class StructuralPCRFPlugin(BaseFeaturePlugin):
    ...
    def run_standalone(self, model: PreTrainedModel, tokenizer: PreTrainedTokenizer, splits: Dict[str, List[ClozeQAExample]], cfg: Any) -> Any:
        ...
        for idx in range(num_layers):
            ...
            # Continuous reliability mapped using the calibrated depth-adaptive beta value
...
class DerivativeRegularizer(BaseFeaturePlugin):
    ...
    def run_standalone(self, model: PreTrainedModel, tokenizer: PreTrainedTokenizer, splits: Dict[str, List[ClozeQAExample]], cfg: Any) -> Any:
        ...
        deriv_plugin = DerivativePlugin()
        deriv_results = deriv_plugin.run_standalone(model, tokenizer, splits, cfg)
        weights = [max(0.0, x["delta_prob"]) for x in deriv_results]
        
        sum_w = sum(weights)
        if sum_w > 0:
            weights = [w / sum_w for w in weights]
            
        logger.info("Initializing Derivative-Weighted SFT Regularization training pass (CDL v2)...")
        
        reference_model = AutoModelForCausalLM.from_pretrained(model_cfg.model_name)
        reference_model.to(model_cfg.device)
        reference_model.eval()
        
        # Section 5: Training Data Fix (Anti Over-steering)
        # We construct curriculum prioritized weights for train data and employ curriculum sampling
        ...
        sampling_probs = [s / sum_scores for s in curriculum_scores] if sum_scores > 0 else [1.0/len(train_examples)]*len(train_examples)
        
        # Sample with curriculum weights (Section 5 requirement)
        sampled_indices = np.random.choice(len(train_examples), size=15, replace=False, p=sampling_probs)
        ...
        # Optimize selected boundary layers only (Section 6 optimization)
        model_mgr = TransformerHookManager(model)
        ref_mgr = TransformerHookManager(reference_model)
        num_layers = len(model_mgr.block_list)
        
        # Identify high derivative + high sensitivity layers
        selected_layers = []
        for d in deriv_results:
            # We target top early and late layers matching Segment constraints
            l_id = d["layer_id"]
            if l_id <= int(0.2 * num_layers) or l_id >= int(0.8 * num_layers):
                selected_layers.append(l_id)
                
        # Prepare params group
        optimized_params = []
        for l_id in selected_layers:
            optimized_params.extend(list(model_mgr.block_list[l_id].parameters()))
            
        if not optimized_params:
            optimized_params = list(model.parameters())
            
        optimizer = torch.optim.AdamW(optimized_params, lr=1e-5)
        ...
        for batch in loader:
            ...
            # Loss identity features extraction
            logits = outputs.logits
            with torch.no_grad():
                ref_outputs = reference_model(input_ids=input_ids, attention_mask=attention_mask)
                ref_logits = ref_outputs.logits
                
            # Alignment components calculation
            # 1. Activation drift regularizer (lambda1)
            ...
            # 3. Target margin constraint (lambda3)
            # Shift predictions for alignment checks
            shift_logits = logits[..., :-1, :].contiguous()
            shift_labels = labels[..., 1:].contiguous()
            ...
            if active_mask.any():
                ...
                # Gather logit of the correct targets
            ...
            # 4. Generation Alignment Loss (lambda4) (Section 3 requirement)
            # Penalty if greedy decoding deviates from baseline correct output
            gen_align_penalty = torch.tensor(0.0, device=device)
            if active_mask.any():
                # We encourage top token of candidate matches baseline target if baseline target matches labels
                pred_tokens = shift_logits[active_mask].argmax(dim=-1)
                # Discrepancy penalty
                gen_align_penalty = (pred_tokens != target_labels).float().mean()
                
            loss = ce_loss + reg_cfg.lambda_reg * reg_penalty + 0.1 * kl_loss + 0.05 * margin_loss + 0.1 * gen_align_penalty
            loss.backward()
            optimizer.step()
...
class SafePCRFController:
    ...
    def compute_promotion_decision(self, baseline_metrics: Dict[str, Any], feature_metrics: Dict[str, Any], feature_name: str) -> FeatureDecisionReport:
        ...
        if "delta_nll_samples" in feature_metrics:
            ...
            # Standard NLL delta must be significantly negative (improving) or at least not degrading
...
This mathematical approach filters out low-impact prompts and prioritizes abstract syntax and logical structures. For example, `{model_name_dynamic}` consistently struggles with advanced computer science and coding syntax prompts (high priority scores), while basic factual lookups (like capitals) remain stable.
...
"""
    ...
    def _generate_conditional_blueprints(output_dir: str, scorecard: Dict[str, Any], baseline_stats: Dict[str, Any]):
        ...
        model_name_dynamic = baseline_stats.get("model_name", "QWEN").upper()
