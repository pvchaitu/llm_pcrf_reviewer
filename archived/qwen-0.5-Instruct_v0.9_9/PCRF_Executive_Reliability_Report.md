...
- **What Happened?**  
  The candidate model's Seen Validation exact-match accuracy was improved by +5.00 percentage points, while the Unseen Validation Generalization accuracy was improved by +5.00 percentage points.  

- **Likelihood & Confidence Behavior:**  
  Generalization Negative Log-Likelihood (NLL) improved (decreased) by 0.9496, and Perplexity (PPL) improved (decreased) by 109.70.  
...
- **What did PCRF Prove in This Run?**  
  ...
  * Semantic target recoveries observed: `2`
  * Contract-clean repairs promoted to customer-facing output: `0`
  * Semantic recoveries withheld due to instruction-contract violations: `2`
  * Safe withhold/abstain decisions executed: `19`
...
| **Active Semantic Target Recoveries** | `2` | Raw semantic improvements found in the SFT candidate. |
| **Active Contract-Clean Repairs Promoted** | `0` | Baseline errors resolved cleanly and promoted with strict EM. |
| **Semantic Recoveries Withheld (Contract)**| `2` | Semantic target recovered, but withheld due to contract/EM violation. |
...
| **Hallucination Exposure Control Rate** | 100.00% | 19 of 19 baseline cases were either repaired by candidate or withheld. |
...
* **Outputs:** Baseline=`__________. Zurich<br><br>What does the sentence "The official` (Risk: 0.3285) | Candidate=`⚠️ Hallucination Risk Detected — Response Withheld for Safety` (Risk: 0.3723)
...
* **Operational Category:** Transition trace display for analysis (wrong_to_correct).
...
* **Router Action:** `abstain_safe_fallback` -> **Served Output:** `⚠️ Hallucination Risk Detected — Response Withheld for Safety`
* **Protected Router Decision Explanation:** *Semantic target recovered by candidate, but output failed strict instruction contract; response withheld for safety.*
...
* **Outputs:** Baseline=`____. <br>A. Gold<br>B. Silver<br>C` (Risk: 0.3379) | Candidate=`⚠️ Hallucination Risk Detected — Response Withheld for Safety` (Risk: 0.4154)
...
| seen_val | 84 | *Complete with one word only: The official capit...* | `Bern` | `__________. Zurich<br><br>W...` | `⚠️ Hallucination Risk Detec...` | 3.7344 |
| seen_val | 86 | *Complete with one word only: The noble element ...* | `Neon` | `____. <br>A. Gold<br>B. Sil...` | `⚠️ Hallucination Risk Detec...` | 10.0625 |
| seen_val | 87 | *Complete with one word only: The volatile eleme...* | `Sulfur` | `__________. <br>A. Sodium<b...` | `⚠️ Hallucination Risk Detec...` | 3.6667 |
| seen_val | 90 | *Complete with one word only: Mechanical acousti...* | `Vacuum` | `medium. <br><br>A) Inertial...` | `⚠️ Hallucination Risk Detec...` | 12.3125 |
| seen_val | 96 | *Complete with one word only: To enforce unique ...* | `Set` | `technique known as ________...` | `⚠️ Hallucination Risk Detec...` | 7.7500 |
| seen_val | 98 | *Complete with one word only: An execution failu...* | `Bug` | `(n) ________.<br>A. Error<b...` | `⚠️ Hallucination Risk Detec...` | 11.8125 |
| seen_val | 99 | *Complete with one word only: A standardized tex...* | `JSON` | `called a(n) ________.<br>A....` | `⚠️ Hallucination Risk Detec...` | 10.9375 |
| seen_val | 100 | *Complete with one word only: The active keyword...* | `import` | `____________.<br>module<br>...` | `⚠️ Hallucination Risk Detec...` | 7.5312 |
| unseen_val | 106 | *Complete with one word only: Mammalian red bloo...* | `Oxygen` | `substances in the body. The...` | `⚠️ Hallucination Risk Detec...` | 9.4375 |
...
| --- | --- | --- | --- | --- | --- | --- | --- |
| Seen Validation NLL | Lower is Better ⬇️ | 4.7107 | 3.6968 | 4.7107 | -1.0138 | +0.0000 | Candidate: Candidate shifted confidence distribution (Lower) (-1.0138), Served: Unchanged (+0.0000). |
| Unseen Validation NLL | Lower is Better ⬇️ | 5.1869 | 4.2373 | 5.1869 | -0.9496 | +0.0000 | Candidate: Candidate shifted confidence distribution (Lower) (-0.9496), Served: Unchanged (+0.0000). |
| Unseen Perplexity (PPL) | Lower is Better ⬇️ | 178.9205 | 69.2234 | 178.9205 | -109.6971 | +0.0000 | Candidate: Candidate shifted confidence distribution (Lower) (-109.6971), Served: Unchanged (+0.0000). |
| Average Cross-Entropy Loss (NLL) | Lower is Better ⬇️ | 4.9488 | 3.9671 | 4.9488 | -0.9817 | +0.0000 | Candidate: Candidate shifted confidence distribution (Lower) (-0.9817), Served: Unchanged (+0.0000). |
...
| Semantic Target Capture | Higher is Better ⬆️ | 52.50% | 57.50% | 52.50% | +5.00% | +0.00% | Candidate: Candidate shifted confidence distribution (Higher) (+0.0500%), Served: Unchanged (+0.0000%). |
| First-Token Target Match | Higher is Better ⬆️ | 37.50% | 47.50% | 37.50% | +10.00% | +0.00% | Candidate: Candidate shifted confidence distribution (Higher) (+0.1000%), Served: Unchanged (+0.0000%). |
...
| Seen Exact-Match Accuracy | Higher is Better ⬆️ | 55.00% | 60.00% | 55.00% | +5.00% | +0.00% | Candidate: Optimized under calibration (+0.0500%), Served: Confidence profile stable (+0.0000%). |
| Unseen Generalization Accuracy | Higher is Better ⬆️ | 50.00% | 55.00% | 50.00% | +5.00% | +0.00% | Candidate: Optimized under calibration (+0.0500%), Served: Confidence profile stable (+0.0000%). |
