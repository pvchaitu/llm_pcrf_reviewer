...
# ==============================================================================
# PHASE 1 & 2 NEW HELPERS (CONTRACT REPAIRS, ROUTER GOVERNANCE, LAYER PROVENANCE, CLI)
# ==============================================================================

def is_contract_clean_candidate_repair(row: Dict[str, Any]) -> bool:
    """
    A production-safe repair requires:
    - baseline failed semantic target capture
    - candidate captured semantic target
    - candidate did not violate instruction contract
    - candidate passed strict target-only exact match
    """
...
def validate_layer_selection_consistency(
    ...
    """
    Validates that all exported artifacts describe the same intervention layer set.
    """
...
def compute_layer_selection_result(
    ...
    """
    Computes empirical, structural/Birnbaum, combined-risk, and final selected layers
...
def parse_cli_args(cli_args=None) -> argparse.Namespace:
    """
    Execution mode switch and optional external dataset selector.
    Jupyter notebook safe.
    """
    parser = argparse.ArgumentParser(
        description="PCRF Transformer Reliability Suite"
    )
    parser.add_argument(
        "mode",
        nargs="?",
        ...
        default="full",
        help="Execution mode: baseline runs baseline-only evaluation; full runs baseline + PCRF (default)."
    )
    parser.add_argument(
        "--dataset",
        type=str,
        default=None,
        help="Path to optional external CSV dataset file."
    )

    # Notebook environment protection
    is_notebook = False
    try:
        if any("ipykernel" in arg for arg in sys.argv) or "ipykernel" in sys.modules:
            is_notebook = True
    except Exception:
        pass
...
# ==============================================================================
# DATASET VALIDATION & LOADING PLUGINS (FEATURE 2)
# ==============================================================================
...
def format_dataset_validation_error(errors: List[str]) -> str:
    ...
    err_str += " - train: Used for adaptation or regularization paths when the full PCRF experiment executes training components.\n"
    ...
    try:
        with open(dataset_path, mode='r', encoding='utf-8') as f:
            ...
            for row in reader:
                ...
                # Validate prompt and target are non-empty strings
...
def load_external_cloze_dataset(dataset_path: str) -> Dict[str, List[ClozeQAExample]]:
    ...
    # Maintain dual backward-compatibility aliases
...
def get_dataset_or_default(dataset_path: Optional[str]) -> Tuple[Dict[str, List[ClozeQAExample]], Dict[str, Any]]:
    ...
    if dataset_path is not None:
        ...
        # Ensure ood/ood_test key alignment
...
def apply_customer_safe_outputs_to_row(
    ...
    # Candidate masking when enabled
    if mask_flag:
...
# ==============================================================================
# TYPED CONFIGURATIONS & COMPLIANCE RULES (PART A)
# ==============================================================================
...
class PromotionGateConfig:
    ...
    # PART A — COMPLIANCE GATE CONTROLS
...
# ==============================================================================
# CORE UTILITY & HALLUCINATION CLASSIFICATION HELPERS
# ==============================================================================
...
# ==============================================================================
# STATS & REPORTING INSTRUMENTATION UTILITIES
# ==============================================================================
...
def validate_customer_safe_output_masking(
    ...
    """
    Verifies output masking consistency rules.
    """
    issues = []
...
def assert_no_raw_hallucinated_outputs_in_customer_report(
    report_text: str, 
    trace_rows: List[Dict[str, Any]]
) -> str:
    """Verifies that no raw hallucinated substrings leaked into the report document."""
    leaks = []
    for r in trace_rows:
...
# ==============================================================================
# HARDWARE COMPUTE PROFILING SUB-SYSTEM
# ==============================================================================
...
# ==============================================================================
# SECTION B. TYPED CONFIGURATIONS (DEFINED BEFORE PCRFCONFIG TO PREVENT NameError)
# ==============================================================================
...
class StructuralPCRFPlugin(BaseFeaturePlugin):
    ...
    def run_standalone(self, model: PreTrainedModel, tokenizer: PreTrainedTokenizer, splits: Dict[str, List[ClozeQAExample]], cfg: PCRFConfig) -> Any:
        ...
        # Calculate canonical LayerSelectionResult (SINGLE SOURCE OF TRUTH - Phase 1 Fix 3)
...
def build_structural_formula_trace(
    ...
    metric_roles = {
        ...
        "Worst-k Bottleneck Risk": "localized adapter targeting signal"
    }
    ...
    return StructuralReliabilityFormulaTrace(
        ...
        text_series_formula=r"R_{sys}^{series} = \prod_{l=1}^{L} r_l",
...
class ProtectedRouter:
    ...
    def route_inference(self, row: Dict[str, Any]) -> Tuple[str, str]:
        ...
        if base_correct == 1 and cand_correct == 0:
            ...
            # Check contract-clean repair condition
            ...
            self.both_wrong += 1
            decision = "abstain_safe_fallback"  # Fail closed for wrong-to-wrong
...
def generate_router_decision_reason(row: Dict[str, Any]) -> str:
    ...
    if b_corr == 1 and c_corr == 0:
        ...
        if decision == "use_candidate":
            return "Contract-clean candidate repair promoted."
        elif decision == "abstain_safe_fallback":
            return "Semantic target recovered by candidate, but output failed strict instruction contract; response withheld for safety."
        else:
            return "Candidate repair not promoted due to higher risk/contract failure."
    elif b_corr == 0 and c_corr == 0:
        if decision == "use_baseline" or decision == "abstain_safe_fallback":
...
def make_metrics_at_a_glance_table(summary: ExperimentComputedSummary, cfg: PCRFConfig) -> str:
    # PART B — METRIC TABLE INCORPORATING READING GUIDANCE (Candidate Delta vs Served Delta)
    metrics = [
        summary.seen_nll,
        summary.unseen_nll,
        summary.unseen_ppl,
    ...
    for m in metrics:
        ...
        if abs(cand_delta_raw) > 1e-7 and abs(serv_delta_raw) < 1e-7:
            guidance = "Candidate moved, but governed served output remained flat. Inspect Candidate Delta for diagnostic improvement and router sections for why the improvement was or was not exposed."
...
def make_layer_sensitivity_section(layer_derivatives: List[Dict[str, Any]], layer_breakdown: List[Dict[str, Any]], cfg: PCRFConfig, canonical_selected_layers: List[int]) -> str:
    policy = cfg.reporting_cfg.bottleneck_selection_policy
...
| **Active Semantic Target Recoveries** | `{stats['semantic_repairs_observed']}` | Raw semantic improvements found in the SFT candidate. |
...
| **Hallucination Exposure Control Rate** | {exposure_control_rate * 100:.2f}% | {exposure_control_count} of {stats['total_b_hallucinations']} baseline cases were either repaired by candidate or withheld. |
...
def make_protected_router_benefit_accounting(summary: ExperimentComputedSummary) -> str:
    ...
    if repaired > 0:
        accounting += f"* **Generalization Repair:** Promoted {repaired} successful contract-clean repair(s) into active serving streams.\n"
    else:
...
| **Contract-Clean Repairs Promoted** | `{repaired}` | Upgrade to candidate on verified contract-clean repair |
...
class SafePCRFController:
    ...
    def run_gating_checks(
        ...
        # PART D — RECLASSIFY CONTRACT GATES AS DIAGNOSTIC BY DEFAULT (D1 & D2)
        ...
        checks.append(GateCheck(
            ...
            explanation="Both baseline and candidate violate the strict output contract. This indicates model-family output behavior rather than a PCRF regression. Metric retained for debugging and output-control planning." if not self.gate_cfg.enforce_instruction_contract_gate else f"Candidate instruction-contract violation rate ({cand_inst_violation*100:.2f}%) must be below ceiling ({self.gate_cfg.max_instruction_violation_rate_for_promotion*100:.1f}%)."
        ))
...
def compute_experiment_summary(
    ...
    oversteers_prevented = sum(1 for r in trace_rows if r["baseline_correct"] == 0 and r["candidate_correct"] == 0 and r["router_decision"] in ["use_baseline", "abstain_safe_fallback"])
...
class ExecutiveReportGenerator:
    ...
    def generate_report(
        ...
        # Separate validation routing from production enforcement (Phase 1 Fix 5)
        deployment_recommendation = "MEASUREMENT_ONLY"
        correct_to_wrong_count = summary.transition_counts.get("correct->wrong", 0)
        ...
        if (summary.final_direct_promotion_decision in ["SAFE_TO_APPLY", "PROMOTED", "PROMOTED_PATH_C"] 
            and correct_to_wrong_count == 0 
            and r_sys_chain >= cfg.gate_cfg.structural_gating_floor 
            and not bypass_dominated):
            deployment_recommendation = "SAFE_TO_APPLY"

        router_gov = resolve_router_governance_status(
        ...
        # Validate report claims securely returning structured issues
        ...
        # Read layers interventions from dynamic files
        layer_derivatives = []
...
{sensitivity_sec}"""
        ...
        # PART B9 Note explaining flat Served Delta
        delta_explanation_note = (
            "\n\n**Reading the Metrics Scoreboard:**\n"
            "* **Candidate Delta** indicates raw diagnostic model representation movement under optimization.\n"
            "* **Served Delta** reflects the governed production-facing impact after Protected Router decisions have been applied.\n"
            "* A Served Delta of `0%` is often a *safety validation win* showing that representational regressions were successfully intervened upon "
            "and baseline fallbacks were deployed.\n"
        )
...
def check_for_duplicate_symbols(filepath: str) -> List[str]:
    """AST symbol auditor parsing structural class and function boundaries."""
    import ast
...
def run_post_experiment_self_checks(
    ...
    # Layer Selection Provenance Assertion (Phase 1 Fix 3)
    if not layer_consistency["is_consistent"]:
    ...
    # Hard Gating Constraints
    cand_iv_rate = summary.instruction_violation.candidate
    strict_em = summary.strict_em.candidate
    status = summary.final_direct_promotion_decision

    # Check validation triggers depending on whether contract gates are active (D1/D2/PART A)
    if cfg.gate_cfg.enforce_instruction_contract_gate or cfg.gate_cfg.enforce_strict_em_repair_gate:
        if (cfg.gate_cfg.enforce_instruction_contract_gate and cand_iv_rate > cfg.gate_cfg.max_instruction_violation_rate_for_promotion) or \
           (cfg.gate_cfg.enforce_strict_em_repair_gate and strict_em < cfg.gate_cfg.min_strict_em_for_direct_promotion):
    ...
    # Protected Router wrong-to-wrong verification
