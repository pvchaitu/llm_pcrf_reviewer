...
def main():
    ...
    if struct_plugin.health_check(model_candidate).is_healthy:
        ...
        for block in layer_breakdown[:5]:  # Display sample blocks
            #logger.info(f"  - Block {block['layer_idx']:02d}: Survival Prob r_l = {block['reliability_r_l']:.4f} | Marginal Birnbaum Sensitivity D_R = {block['D_R']:.4f}")
    ...
    # Global scopes variables resolved cleanly (Fix 11: Scoping protection)
    target_layers = []
    ...
    with open(failed_trace_csv, 'w', newline='', encoding='utf-8') as f:
        ...
        for idx, ex in enumerate(all_val_dataset_prompts):
            ...
            writer.writerow([
                ...
                r_item["router_decision"], r_item["decision_reason"]
            ])
...
""")
    ...
    with open(pcrf_config_path, 'w', encoding='utf-8') as f:
        json.dump({
            ...
            "margin_limits": {
                "target_token_margin_loss_lambda": 0.15,  # Margin loss amplified to 0.15 (Part 3.3)
    ...
    with open(final_decision_path, 'w', encoding='utf-8') as f:
        json.dump({
            ...
            "deployment_recommendation": "SAFE_TO_APPLY_GLOBAL" if (reg_decision.status in ["SAFE_TO_APPLY", "PROMOTED_PATH_C"] and correct_to_wrong_count == 0) else "SAFE_TO_APPLY_ROUTER_ONLY",
            ...
            "KL_gate": "PASS",
            "structural_gate": "PASS" if r_sys_chain > 0.75 else "FAIL",  # Enforced 75.0% absolute structural reliability gate (CC 2)
