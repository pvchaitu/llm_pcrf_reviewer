...
def parse_cli_args() -> argparse.Namespace:
    ...
    Supported modes:
    - baseline: runs only baseline model evaluation and baseline artifacts.
    - full: runs existing full PCRF experiment path.
    """
...
def format_dataset_validation_error(errors: List[str]) -> str:
    ...
    err_str += " - train: Used for fitting, adaptation, or regularization paths when the full PCRF experiment executes training-related components.\n"
    ...
    err_str += " - ood: Used to measure out-of-distribution or harder transfer behavior so the reliability suite can distinguish in-distribution reliability from broader deployment risk.\n"
    err_str += "=" * 90 + "\n"
...
def write_baseline_only_artifacts(
    ...
    """
    Writes minimal baseline-only artifacts.

    This intentionally avoids:
    - PCRF derivative estimation
    - curriculum curation
    - structural PCRF analysis
    - candidate regularization
    - protected router comparison
    """
    ...
    md_path = os.path.join(output_dir, "Baseline_Only_Report.md")
    with open(md_path, "w", encoding="utf-8") as f:
        ...
        f.write("- Candidate Regularization Executed: No\n")
        f.write("- Protected Router Executed: No\n\n")

        # Dataset Source metadata section near the top
        ...
        # Enhanced row-level prompts generation hallucination audit section (FEATURE 1)
        ...
        all_preds = []
        for p in base_seen_predictions:
            all_preds.append((p, "seen_val"))
        for p in base_unseen_predictions:
            all_preds.append((p, "unseen_val"))

        for p, split_name in all_preds:
            pid = p.get("id", "N/A")
            ...
            f.write(f"| {pid} | {split_name} | {p_prompt} | {p_gen} | {p_expected} | {p_actual} | {match_yes_no} | {hall_yes_no} |\n")

    logger.info(f"Baseline-only report and row audits generated at: {md_path}")
...
class StructuralPCRFPlugin(BaseFeaturePlugin):
    ...
    def run_standalone(self, model: PreTrainedModel, tokenizer: PreTrainedTokenizer, splits: Dict[str, List[ClozeQAExample]], cfg: PCRFConfig) -> Any:
        ...
        # Calculate canonical LayerSelectionResult (SINGLE SOURCE OF TRUTH)
