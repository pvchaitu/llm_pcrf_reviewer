...
| **Seen Validation Loss (NLL)** | 4.22905 | 3.68145 | -0.54761 |
| **Unseen Validation Loss (NLL)** | 4.21506 | 3.86218 | -0.35288 |
| **Unseen Perplexity (PPL)** | 67.70 | 47.57 | -20.13 |
...
|---|---|---|---|---|---|---|
| seen_val | 81 | *Complete with one word only: The official capital city of South Korea is* | `Seoul` | `___________.
A.` | `Seoul. Seoul is the official` | 1.6797 |
| seen_val | 84 | *Complete with one word only: The official capital city of Switzerland is* | `Bern` | `___________.
A.` | `___________.
A.` | 3.5469 |
| seen_val | 86 | *Complete with one word only: The noble element designated by atomic number 10 is* | `Neon` | `____
A. Sodium` | `____
A. Sodium` | 9.9375 |
| seen_val | 87 | *Complete with one word only: The volatile element designated by atomic number 16 is* | `Sulfur` | `____
A. Sodium` | `____
A. Sodium` | 3.3125 |
| seen_val | 89 | *Complete with one word only: The yellow dwarf star supporting life at the center of our solar system is the* | `Sun` | `closest star to Earth, and` | `closest star to Earth, and` | 3.2031 |
| seen_val | 90 | *Complete with one word only: Mechanical acoustics are completely incapable of moving across a spatial* | `Vacuum` | `medium. They are only able` | `medium. They are only able` | 12.5000 |
| seen_val | 96 | *Complete with one word only: To enforce unique constraints with no duplicated items, algorithms utilize a* | `Set` | `technique called "hashing."` | `technique called hashing. Hashing` | 6.5625 |
| seen_val | 97 | *Complete with one word only: The hypermedia syntax used to format layout documents across the World Wide Web is* | `HTML` | `called ____________.
A` | `called ____________.
A` | 3.8750 |
| seen_val | 98 | *Complete with one word only: An execution failure originating from incorrect program logic is called a* | `Bug` | `(n) ________.
A` | `(n) ________.
A` | 10.6875 |
| seen_val | 99 | *Complete with one word only: A standardized text notation representing complex structural records is* | `JSON` | `called a ____.
A.` | `called a ____.
A.` | 10.4375 |
