...
- **What Happened?**  
  The SFT candidate's Seen Validation EM accuracy was improved by +2.40 percentage points, while the Unseen SFT Generalization accuracy was unchanged.  
...
- **What did PCRF Prove in This Run?**  
  ...
  - **PCRF Hallucination Exposure Control:** 100.00% of 95 baseline hallucination/target-failure cases were controlled through 100 protected router interventions.
...
| :--- | :---: | :--- |
| **Observed Risk Events** | `100` | All validation prompts triggering baseline failure or candidate degradation. |
| **Contained Risk Events** | `100` | Total safety interventions successfully managed by Protected Router. |
| **Served Risk Events** | `0` | Safety-withheld or incorrect completions exposed to served streams. |
| **Safe Abstains** | `92` | Unsafe outputs withheld and mapped cleanly to fallback states. |
...
> 
> * **Governed Accuracy (Primary Customer Metric):** `63.20%`  
...
> * **Safe Abstains Executed:** `92`  
> 
> **Service Impact Narrative:**  
> Governed outcomes remained protected despite candidate degradation events during SFT evaluation. The Protected Router successfully insulated the final served endpoint, maintaining served quality at **63.20%** while preventing degraded candidate outputs from reaching users.
...
| Candidate Regression Review | 🟢 PASS | DIAGNOSTIC_ONLY | Observed Regressions: 5, Observed Repairs: 3, Net Delta: -2 | N/A | Model evaluation only. Captures raw candidate parameter variance before PCRF routing. |
...
| Generalization Non-Degradation Guard | 🟡 WARNING | WARNING | -0.80% | 0.00% | Generalization monitoring warning: raw SFT candidate unseen validation exact-match changed by -0.80% versus baseline. This is reported as candidate-side generalization telemetry only and does not block deployment when Protected Router served outputs remain governed and regression exposure is contained. |
...
| Served Seen Accuracy Non-Inferiority Margin | 🟢 PASS | HIGH | 0.80% | 1.00% | Served seen accuracy drop after Protected Router governance (0.80%) vs non-inferiority margin (1.0%). This gate is deployment-facing and evaluates governed served output, not raw candidate-only movement. |
| Served Seen Accuracy Degradation Budget | 🟢 PASS | CRITICAL | 0.80% | 3.00% | Served seen accuracy degradation after Protected Router governance (0.80%) vs deployment budget (3.0%). This remains a blocking gate only when the governed served stream degrades beyond the allowed budget. |
| Candidate Unseen Accuracy Improvement Review | 🟡 WARNING | WARNING | -0.80% | 2.00% | Candidate-side unseen accuracy improvement was -0.80% vs target 2.0%. This is reported as raw SFT generalization telemetry only and does not block deployment when served outputs remain governed. |
| Served Unseen Generalization Preservation | 🔴 FAIL | HIGH | -0.80% | 0.00% | Served unseen accuracy movement after Protected Router governance was -0.80% versus baseline. This deployment-facing gate verifies that governed served output does not degrade on unseen validation. |
| Generalization Non-Degradation Guard | 🟡 WARNING | WARNING | -0.80% | 0.00% | Generalization monitoring warning: raw SFT candidate unseen validation exact-match changed by -0.80% versus baseline. This is reported as candidate-side generalization telemetry only and does not block deployment when Protected Router served outputs remain governed and regression exposure is contained. |
...
* **Outputs:** Baseline=`'viewer' } The user role is viewer. H...` (Risk: 0.1981) | SFT Candidate=`'admin', 'permissions': ['read_audit_...` (Risk: 0.5722)
...
* **Outputs:** Baseline=`low<br><br>What is the risk level of ...` (Risk: 0.4121) | SFT Candidate=`low<br><br>What is the risk level of ...` (Risk: 0.4474)
...
* **Expected Target:** `low`
* **Outputs:** Baseline=`low<br><br>What is the risk level of ...` (Risk: 0.4102) | SFT Candidate=`low<br><br>What is the risk level of ...` (Risk: 0.4474)
...
* **Outputs:** Baseline=`'rejected', 'reason': 'user rejected ...` (Risk: 0.3852) | SFT Candidate=`'rejected', 'reason': 'user declined ...` (Risk: 0.2503)
...
| seen_val | 206 | *JSON payload: { 'event': 'IAMPolicyModified', '...* | `medium` | `What is the risk of this IA...` | `What is the risk of this IA...` | 7.2812 |
...
| seen_val | 223 | *Extract the user role (admin/editor/viewer) fro...* | `viewer` | `'viewer' } The user role is...` | `'admin', 'permissions': ['r...` | 20.3750 |
...
| Unseen Validation NLL | Lower is Better ⬇️ | 13.5418 | 8.3483 | 13.5418 | -5.1935 | Favorable | +0.0000 | Flat (Repair Opportunity Withheld) | SFT candidate improved Unseen Validation NLL by -5.1935, but promotion was withheld due to strict structural floor safety policies. Baseline output was retained for safety. |
