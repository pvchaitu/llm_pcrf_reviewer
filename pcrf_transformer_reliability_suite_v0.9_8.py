...
def get_public_served_output(row: Dict[str, Any], cfg: Any = None) -> str:
    """
    Returns strictly governed served output.
    No undefined behavior allowed.
    """

    # ✅ Unresolved hallucination → abstain
    ...
    # ✅ Candidate route
    if decision in ["use_candidate", "candidate"]:
        return get_public_candidate_output(row, cfg)

    # ✅ Baseline route
    if decision in ["use_baseline", "baseline"]:
    ...
    # ✅ FINAL SAFETY FALLBACK (no uncontrolled candidate exposure)
    return SAFETY_WITHHELD_RESPONSE
...
def compute_hallucination_exposure_control_stats(
    ...
    """
    Strictly measures hallucination exposure control:
    ONLY true repairs + safe abstains are counted.
    """

    total = sum(1 for r in trace_rows if is_baseline_hallucination(r))

    repairs = sum(
        1 for r in trace_rows
        if is_baseline_hallucination(r) and not is_candidate_hallucination(r)
    )

    safe_withholds = sum(
        1 for r in trace_rows
        if is_baseline_hallucination(r) and is_candidate_hallucination(r)
    )

    # ✅ STRICT CONTROL LOGIC
    controlled = sum(
        1 for r in trace_rows
        if is_baseline_hallucination(r) and (
            (
                r.get("router_decision") == "use_candidate"
                and not is_candidate_hallucination(r)
            )
            or r.get("router_decision") == "abstain_safe_fallback"
        )
    )

    rate = controlled / total if total > 0 else 0.0

    oversteers = sum(1 for r in trace_rows if r.get("oversteer_prevented", False))
    regressions = sum(1 for r in trace_rows if r.get("regression_blocked", False))

    return {
        "total_b_hallucinations": total,
        "repairs_promoted": repairs,
        "safe_hallucination_withholds": safe_withholds,
        "hallucination_exposure_control_count": controlled,
        "hallucination_exposure_control_rate": rate,
        "oversteers_prevented": oversteers,
        "regressions_blocked": regressions,
        "net_interventions": controlled
    }
...
def make_customer_executive_summary_box(
    ...
    # ✅ CRITICAL: use precomputed stats ONLY
    stats = summary.hallucination_stats or {}

    rate = stats.get("hallucination_exposure_control_rate", 0.0)
    total = stats.get("total_b_hallucinations", 0)
    interventions = stats.get("net_interventions", 0)
...
- **PCRF Hallucination Exposure Control:**  
  {rate*100:.2f}% of {total} baseline hallucinations were safely controlled via {interventions} router interventions (repairs promoted: {repairs}).
