...
- **PCRF Hallucination Exposure Control:**  
  100.00% of 16 baseline hallucinations were safely controlled via 16 router interventions (repairs promoted: 0).
...
|---|:---:|---|
| **Total Baseline Hallucinations Found** | `16` | Validation prompts where baseline failed to capture semantic target. |
...
| **Hallucination Exposure Control Rate** | `100.00%` | 16 of 16 baseline hallucination cases were either repaired by PCRF candidate or withheld through safe fallback. |
| **Net Gateway Interventions** | `16` | Overall cases actively guarded by the Protected Router (100% active coverage). |
...
#### Showcase Case 2: ID 082 (seen_val)
...
* **Router Action:** `abstain_safe_fallback` -> **Served Output:** `__________. Zurich<br><br>What does the sentence "The official`
...
* **Router Action:** `abstain_safe_fallback` -> **Served Output:** `____. <br>A. Gold<br>B. Silver<br>C`
